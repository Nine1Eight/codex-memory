...
FEDERATION_TOKEN = os.environ.get("FEDERATION_TOKEN") or secrets.token_urlsafe(32)
...
SPACES = [
    {
        ...
        "emoji": "⚔️",
        "desc": "Playable arena core with match creation, event tracking, and federation push.",
    },
    {
        ...
        "emoji": "🏆",
        "desc": "Trophy vault and trace archive node.",
    },
    {
        ...
        "emoji": "📊",
        "desc": "Federated leaderboard and player tracking node.",
    },
]
...
URLS = {s["role"]: runtime_url(s["name"]) for s in SPACES}
...
DEFAULT_ROLE = "__ROLE__"
DEFAULT_PEERS = "__PEERS__"

APP_ROLE = os.getenv("SPACE_ROLE", DEFAULT_ROLE)
PUBLIC_SPACE_NAME = os.getenv("PUBLIC_SPACE_NAME", "__SPACE_NAME__")
FEDERATION_PEERS = [
    x.strip().rstrip("/")
...
RUN mkdir -p /data && useradd -m -u 1000 user && chown -R user:user /app /data
USER user
...
build_root = Path("hf_multi_space_build")
build_root.mkdir(exist_ok=True)

print("Federation token generated/loaded. Do not paste it into chat.")
print("Deploying multi-space Docker package...")
...
for space in SPACES:
    ...
    peers = ",".join(URLS[r["role"]] for r in SPACES if r["role"] != role)

    app_py = (
        APP_TEMPLATE
        .replace("__ROLE__", role)
        .replace("__SPACE_NAME__", name)
...
'''
    ...
    (folder / "requirements.txt").write_text(REQS, encoding="utf-8")
    (folder / ".dockerignore").write_text(DOCKERIGNORE, encoding="utf-8")
    (folder / "app.py").write_text(app_py, encoding="utf-8")

    print(f"\n=== Creating/updating {repo_id} ===")
    api.create_repo(
        repo_id=repo_id,
        repo_type="space",
        space_sdk="docker",
    ...
    variables = {
        "SPACE_ROLE": role,
        "PUBLIC_SPACE_NAME": name,
        "APP_STORAGE_DIR": "/data",
        "FEDERATION_PEERS": peers,
        "ARENA_SPACE_URL": URLS["arena"],
        "VAULT_SPACE_URL": URLS["vault"],
        "LEADERBOARD_SPACE_URL": URLS["leaderboard"],
    }
    ...
    for key, value in variables.items():
        try:
            ...
            print(f"warning: variable not set by API: {key}: {e}")

    try:
        api.add_space_secret(repo_id=repo_id, key="FEDERATION_TOKEN", value=FEDERATION_TOKEN, token=TOKEN)
        print("secret set: FEDERATION_TOKEN")
    except Exception as e:
        print(f"warning: secret not set by API: {e}")
    ...
    print(f"repo:    https://huggingface.co/spaces/{repo_id}")
    print(f"runtime: {URLS[role]}")

print("\nDONE")
print("Arena:", URLS["arena"])
print("Vault:", URLS["vault"])
print("Leaderboard:", URLS["leaderboard"])
